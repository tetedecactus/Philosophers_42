...
// 		if (pthread_create(&data->philo[i].philo_th, NULL, routine, &data->philo[i]) != 0)
// 			printf("Error when thread %d create\n", i);
// 		if (pthread_create(&data->philo[i].monitor, NULL, monitore, &data->philo[i]))
...
// 		if (pthread_join(data->philo[i].monitor, NULL) != 0)
// 		printf("Error threads join\n");
// 	}
// }
